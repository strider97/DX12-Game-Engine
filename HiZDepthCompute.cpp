...
void HiZDepthCompute::loadPipeline()
{
    createRootSignature();
    createPSO();
    createCommandList();
...
int HiZDepthCompute::WIDTH = 1366;
int HiZDepthCompute::HEIGHT = 768 * 2;

void HiZDepthCompute::loadResources()
{   
    UINT textureWidth = WIDTH; // Adjust the width and height as needed
    UINT textureHeight = HEIGHT;
    DXGI_FORMAT textureFormat = DXGI_FORMAT_R32_FLOAT; // Choose a suitable format
...
void HiZDepthCompute::WaitForComputeTask(UINT64 &m_fenceValue, 
    ComPtr<ID3D12CommandQueue> &computeCommandQueue, HANDLE &m_fenceEvent, ComPtr<ID3D12Fence> &m_fence)
{
...
void HiZDepthCompute::createRootSignature() {
    ...
    sampler.Filter = D3D12_FILTER_MINIMUM_MIN_LINEAR_MAG_MIP_POINT;
...
void HiZDepthCompute::executeTasks(UINT64 &m_fenceValue, ComPtr<ID3D12CommandQueue> &computeCommandQueue, 
    HANDLE &m_fenceEvent, ComPtr<ID3D12Fence> &m_fence) 
{
    ...
    commandList->SetComputeRootDescriptorTable(1, depthTextureHandle);

    const UINT MAX_LOD_LEVELS = 7;
    const UINT dispatchWidth = WIDTH;
    const UINT dispatchHeight = HEIGHT / 2;

    for(int i = 0; i < MAX_LOD_LEVELS; i++) {
        UINT LOD = i;
        UINT powerOf2 = pow(2, LOD);
        dispatchFor(LOD, 
            (dispatchWidth / powerOf2 + 32 - 1) / 32, 
            (dispatchHeight / powerOf2 + 32 - 1) / 32
        );
        // WaitForComputeTask(m_fenceValue, computeCommandQueue, m_fenceEvent, m_fence);
    }
}

void HiZDepthCompute::dispatchFor(int level, UINT dispatchWidth, UINT dispatchHeight) {
    commandList->SetComputeRoot32BitConstant(2, level, 0);
