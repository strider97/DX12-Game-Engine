...
void BasicGameEngine::OnInit()
{
    ...
    ssrPass->setResources(m_gbufferRtvHeap, m_gbufferDsvHeap, m_cbvHeap, lightingPass->backBuffersHeap, iblResources, noiseTexture, HiZDephtComputePass->textureHeap);
...
void BasicGameEngine::LoadPipelineAssets()
{
    // Create a root signature consisting of a descriptor table with a single CBV.
    {
        ...
        samplerPreFilter.AddressU = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
        samplerPreFilter.AddressV = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
        samplerPreFilter.AddressW = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
        samplerPreFilter.MipLODBias = 0;
        samplerPreFilter.MaxAnisotropy = 0;
        samplerPreFilter.ComparisonFunc = D3D12_COMPARISON_FUNC_NEVER;
        // sampler.BorderColor = D3D12_STATIC_BORDER_COLOR_TRANSPARENT_BLACK;
    ...
    // Create the pipeline state, which includes compiling and loading shaders.
    {
        ...
        ThrowIfFailed(D3DCompileFromFile(GetAssetFullPath(L"shaders.hlsl").c_str(), nullptr, nullptr, "GBufferRenderTargets", "ps_5_0", compileFlags, 0, &pixelShader, nullptr));
...
void BasicGameEngine::PopulateCommandList()
{
    ...
    for(int i=0; i<3; i++) {
        m_commandList->ClearRenderTargetView(rtvHandle_, clearColor, 0, nullptr);
        rtvHandle_.Offset(m_rtvDescriptorSize);
    }
